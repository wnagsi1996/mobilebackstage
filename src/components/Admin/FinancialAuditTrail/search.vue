		...
		<div class="search-info">
			<van-form>
				<van-cell-group>
					...
					:value="fromorder.channel.text" label="订单类别" placeholder="订单类别" @click="showPickerList(1)"/>
				</van-cell-group>
				<van-cell-group>
					<van-field readonly clickable right-icon="arrow-down" name="picker" 
					:value="fromorder.currency.text" label="货币类型" placeholder="货币类型" @click="showPickerList(0)"/>
				</van-cell-group>
				<van-cell-group>
					<van-field readonly clickable right-icon="arrow-down" name="picker" 
...
<script>
	...
	import {cwChannel,cwCurrency,cwOrderStaus,cwOrderType} from '@/utils/searchCondition.js' //引入公共的财务订单类别/财务货币类型/财务订单状态/财务订单类型
	export default({
			...
			async showPickerList(i){
				...
				switch(i){
					case 0: //货币类型
						...
						break;
					case 1:  //订单类别
						this.columns=cwChannel
						break;
					case 2:  //订单类型
						this.columns=cwOrderType;
						break;
					case 4:  //订单状态
