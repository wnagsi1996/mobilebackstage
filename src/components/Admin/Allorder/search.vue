		...
		<div class="search-info">
			<van-form>
				...
				<van-cell-group>
					...
					:value="fromorder.fduser.text" label="放单人员" placeholder="放单人员" @click="showPickerList(1)"/>
				</van-cell-group>
				<van-cell-group>
					<van-field readonly clickable right-icon="arrow-down" name="picker" 
					:value="fromorder.jduser.text" label="接单人员" placeholder="接单人员" @click="showPickerList(0)"/>
				</van-cell-group>
				<van-cell-group>
					<van-field readonly clickable right-icon="arrow-down" name="picker" 
	...
	export default({
		...
		methods:{
			...
			async showPickerList(i){
				...
				switch(i){
					case 0: //接单成员
						...
						break;
					case 1:  //放单成员
						if(this.fduserdata.length==0){
							await this.$store.dispatch('userList/getFdUserList');
						}
						this.columns=this.fduserdata
						break;
					case 2:  //订单类型
			...
			onConfirm(val){
				switch(this.iftype){
					case 0:
						this.fromorder.jduser.text=val.text;
						this.fromorder.jduser.value=val.value;
						break;
					case 1:
						this.fromorder.fduser.text=val.text;
						this.fromorder.fduser.value=val.value;
						break;
					case 2:
