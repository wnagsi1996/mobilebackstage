<template>
	<div>
		...
		<div class="search-info">
			<van-form>
				<!-- <van-cell-group>
					<van-field v-model="fromorder.OrderNo" label="订单号" placeholder="系统单号" />
				</van-cell-group> -->
				<van-cell-group>
					<van-field v-model.trim="fromorder.agentname" label="中介名称" placeholder="中介名称" />
				</van-cell-group>
				<van-cell-group>
					<van-field readonly clickable right-icon="arrow-down" name="picker" 
					:value="fromorder.status.text" label="订单状态" placeholder="订单状态" @click="showPickerList(2)"/>
				</van-cell-group>
				<van-cell-group>
...
<script>
	...
	import {zjOrderType} from '@/utils/searchCondition.js' //引入公共的中介订单状态
	export default({
		name:'Intermediarycommissionstorage-search',
		components:{
			[Button.name]:Button,
			[Popup.name]:Popup,
			[Form.name]:Form,
		...
		methods:{
			...
			async onConfirm(val){
				switch(this.iftype){
					case 2:
						this.fromorder.status.text=val.text;
						this.fromorder.status.value=val.value;
						break;
					case 5:
